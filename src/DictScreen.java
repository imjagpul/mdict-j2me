        ...
        public void run() {
            ...
            if (searchTimerCycle >= 0) {
                if (searchTimerCycle > 10) {
                    searchTimerCycle = -1;
                    boolean b = true;
                    for (int i = 0; i < text.wordCount(); i++) {
                        b &= (textLookups[i].cursor > 0);
                    }
                    if (b) {
                        search(true);
                    }
...
//			keyPressed(getKeyCode(FIRE));
    ...
    public void back() {
        if (state == EDITING && (!currentSearch.empty())) {
            ...
            if (last > 0) {
                historyRecords.removeElementAt(last - 1);
            }
            last--;
            if (last > 0) {
        ...
        if (state == EDITING) {
            ...
            for (int w = 0; w < text.wordCount(); w++) {
                if (textLookups[w].cursor == -1) {
                    ...
                    if (wordPos > 0) {
                        tokenLeftPos = editFont.charsWidth(text.buffer, 0, wordPos);
                    }
